...
def mage_stats(mages: list[dict]) -> dict:
    ...
    max_min = [max_power.get("power", 0), min_power.get("power", 0)]
    avg = int(sum(max_min) / 2)

    return {"max_power": max_min[0], "min_power": max_min[1], "avg_power": avg}


def main():

    artifact = [
        {"name": "Crystal Orb", "power": 85, "type": "selver"},
        {"name": "Fire Staff", "power": 92, "type": "selver"},
    ]
